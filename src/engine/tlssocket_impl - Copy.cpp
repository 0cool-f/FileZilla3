...
#include <libfilezilla/file.hpp> //xxx
#include <libfilezilla/buffer.hpp> //xxx
...
bool CTlsSocketImpl::Init()
{
	...
	/*
	fz::file file(L"d:\\diag.log", fz::file::reading);

	fz::buffer buf;
	file.read(buf.get(file.size()), file.size());
	buf.add(file.size());
	...
	while (!buf.empty()) {
		...
		if (type.back() == 'L') {
			int r = gnutls_x509_trust_list_add_trust_mem(tlist, NULL, &data, GNUTLS_X509_FMT_DER, GNUTLS_TL_USE_IN_TLS | GNUTLS_TL_NO_DUPLICATES, 0);
			if (r >= 0) {
				++loaded[type];
			}
		}
		else {
			int r = gnutls_x509_trust_list_add_trust_mem(tlist, &data, NULL, GNUTLS_X509_FMT_DER, GNUTLS_TL_USE_IN_TLS | GNUTLS_TL_NO_DUPLICATES, 0);
			if (r >= 0) {
				++loaded[type];
			}
		}
...
int CTlsSocketImpl::VerifyCertificate()
{
	...
	auto const start2 = fz::datetime::now();
	if (uses_hostname) {
	...
	auto const stop2 = fz::datetime::now();
	auto const elapsed2 = stop2 - start2;
